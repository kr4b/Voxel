...
impl DynamicTexture {
    ...
    fn new(
        ...
        let memory_requirements =
            unsafe { logical_device.value.get_image_memory_requirements(value) };
    ...
    pub fn update<T>(&mut self, logical_device: &LogicalDevice, command_pool: &CommandPool, queues: &Queues, data: &Vec<T>) {
        let size = (self.width * self.height * self.depth * self.dimensions) as vk::DeviceSize;
        let raw_data = unsafe {
            logical_device
                .value
        ...
        unsafe {
            ...
            logical_device.value.unmap_memory(self.buffer_memory);
        }
    ...
    fn transition_image_layout(
        ...
        let (src_stages, dst_stages, src_access_mask, dst_access_mask) =
